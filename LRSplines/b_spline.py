...
@memoize
def _evaluate_univariate_b_spline(
    x: float, knots: typing.Union[Vector, np.ndarray], degree: int, endpoint=False, r=0
) -> float:
    """
...
def cached_univariate(
    degree: int,
    knots: typing.Union[typing.List[float], np.ndarray],
    endpoint: bool = False,
) -> typing.Callable:
    """
...
class BSpline:
    ...
    def __call__(self, u: float, v: float, r1=0, r2=0) -> float:
        ...
        return (
            ...
            * _evaluate_univariate_b_spline(
                u, self.knots_u, self.degree_u, self.end_u, r1
            )
            * _evaluate_univariate_b_spline(
    ...
    def __eq__(self, other: object) -> bool:
        ...
        if not isinstance(other, BSpline):
            return False
        return np.allclose(self.knots_u, other.knots_u, atol=1.0e-14) and np.allclose(
            self.knots_v, other.knots_v, atol=1.0e-14
        )
    ...
    def intersects(self, element: "Element") -> bool:
        ...
        intersection_u = min(self.knots_u[-1], element.u_max) - max(
            self.knots_u[0], element.u_min
        )
        intersection_v = min(self.knots_v[-1], element.v_max) - max(
            self.knots_v[0], element.v_min
        )
    ...
    def update_weights(self, other: "BSpline") -> None:
        ...
        self.coefficient = (
            self.coefficient * self.weight + other.coefficient * other.weight
        ) / w
        self.weight = w

    def __hash__(self):
        return (
            hash(tuple(self.knots_v)) * self.degree_u
            + hash(tuple(self.knots_u)) * self.degree_v
        )
    ...
    def grad(self, X, Y):
        return np.array(
            [self.__call__(X, Y, r1=1, r2=0), self.__call__(X, Y, r1=0, r2=1)]
        )
